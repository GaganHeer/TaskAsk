...
const handler = (payload, res) => {
    ...
    if (onlyNumbers.test(payload.text)) {
        var taskNumber = parseInt(payload.text);
        pool.connect().then(client => {
            return client.query('SELECT * FROM ASK_TABLE WHERE SERIAL_ID = $1;', [taskNumber])
                .then(res => {
                    client.release();
                    var taskNumberRow = res.rows;
                    if (taskNumberRow.length == 0){
                        ...
                        let taskID = taskNumberRow[0].serial_id;
                        pool.connect().then(client => {
                            return client.query('DELETE FROM ask_table WHERE serial_id = $1;', [taskID])
                                .then(res => {
                                    let deleteMsg = 'Task: '+ taskID +' Successfully Deleted';
